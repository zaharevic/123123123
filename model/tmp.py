...
def get_day_rasp(json1, first_day, last_day):
    first_day_parsed = string_to_datatime(first_day)
    last_day_parsed = string_to_datatime(last_day, '23:59')
    flag = True
    days_tmp = days()
    i = 1
    while flag:
        tmp_data = json1.json()
        for dat in tmp_data:
            lesson_start = string_to_datatime(re.split('[,T]', dat['start'])[0],
                                              re.split('[,T]', dat['start'])[1][:-3])
            if first_day_parsed <= lesson_start <= last_day_parsed:
                lesson_name = dat['title']
                lesson_end = string_to_datatime(re.split('[,T]', dat['end'])[0],
...
def print_calendar():
    ...
    if info != 0:
        data = get_day_rasp(info, '2024-01-01', '2025-01-01')
        print_rasp(data)
